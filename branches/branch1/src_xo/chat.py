...
# Sound Flags
chatSoundFlag = True
soundResumeFlag = True
soundPauseFlag = True

def chatSound(message = None,character = None):
    
    global chatSoundFlag
    global soundPauseFlag
    global soundResumeFlag
...
class chat:
    ...
    def addChat(self,playerName = None,message = ''):
        ...
        if run and (not show_whole_chat_flag):
            sound_thread = threading.Thread(target = chatSound, args=[message,playerName]).start()
...
def showChat(chatText,back_image=None):
    ...
    while run:
        ...
        if ((model.global_time >= 10000) or (i==0 and allowed==True) or (show_whole_chat_flag == True)) and (chatSoundFlag == True):
                if i==0:
                    allowed=False
